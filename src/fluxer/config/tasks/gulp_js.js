...
function Task(gulp, path, config, plugins, functions){
  ...
  gulp.task('js', function(cb){
    plugins.runSequence('js:compile:libs', 'js:compile', 'js:copy-third-party-libs', cb)
  });

  /**
   * Tarea mueve las librerias instaladas por bower a los staticos
   * (gulp js:copy-third-party-libs)
   */
  gulp.task('js:copy-third-party-libs', function(cb){
      return gulp.src(path.frontend.third_party_libs + '/**/*.js')
      .pipe(gulp.dest(path.dest.third_party_libs))
      .on('end', functions.successHandler);
  });
  ...
  gulp.task('js:all', function(cb){
    plugins.runSequence('clean:js', 'js:copy-third-party-libs', 'js:compile:libs', 'js:compile', 'copy:js:libs', cb)
  });
}
